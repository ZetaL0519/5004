...
// *** YOU Write the BoxerComparator instantiated in the line above ***
    ...
    List<Boxer> heavyweights = sortedByWeight
            .stream()
            .filter(i -> i.weightClass == WEIGHT_CLASS.HEAVY)
            .collect(Collectors.toList());
    System.out.println(heavyweights);
  }

  static class BoxerComparator implements Comparator<Boxer> {
    public int compare(Boxer a, Boxer b) {
      return (int) ((int) a.weight - b.weight);
    }
  }
}